...
in terms of the sum of shell cardinalities (weighted by shell-squared
Fourier moduli).  The factor `4·4^(j+1)` comes from the dyadic shell
at level `j+1`; for `j = 0` the single zero-mode gives the degenerate
bound `‖mFourierCoeff f 0‖²`.

This aggregates `sq_norm_lpProjector_succ_le` across all shells. -/
