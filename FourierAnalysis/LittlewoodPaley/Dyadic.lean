...
open UnitAddTorus in
/-- Additivity of the 2D Fourier coefficient under joint integrability. -/
lemma mFourierCoeff_add {f g : 𝕋² → ℂ} (k : Fin 2 → ℤ)
    (hf : MeasureTheory.Integrable (fun t : 𝕋² => mFourier (-k) t • f t))
    (hg : MeasureTheory.Integrable (fun t : 𝕋² => mFourier (-k) t • g t)) :
